...
.reveal {
  opacity: 0;
  transform: perspective(1200px) translateY(28px) rotateX(6deg);
...
/* ============================================================
   ...
   ============================================================ */
.kpi-tilt, .card-motion {
  transform-style: preserve-3d;
  transition: transform 0.15s ease-out, box-shadow 0.3s var(--ease), border-color 0.3s var(--ease);
  will-change: transform;
}
.card-motion { position: relative; overflow: hidden; }
.kpi-tilt::after, .card-motion::after {
  content: "";
  position: absolute;
  inset: 0;
...
.kpi-tilt:hover::after, .card-motion:hover::after { opacity: 1; }
.kpi-tilt:hover, .card-motion:hover {
  border-color: rgba(255, 255, 255, 0.18);
  box-shadow: 0 20px 45px rgba(0, 0, 0, 0.5), 0 0 30px -8px var(--accent-glow);
}

/* ============================================================
   Ambient animated 3D-style background — a few soft, blurred
   gradient orbs drifting slowly behind the whole app.
   ============================================================ */
.ambient-bg {
  position: fixed;
  inset: 0;
...
@keyframes ambientDrift3 {
  0%, 100% { transform: translate3d(0, 0, 0) scale(1); }
  50% { transform: translate3d(-40px, 50px, 0) scale(0.92); }
}

/* Everything else needs to sit above the ambient background layer */
.navbar, .page, .auth-page { position: relative; z-index: 1; }

/* ============================================================
   Small interactive 3D touch on the brand logo (Navbar) — a
   gentle idle float plus a snappier tilt on hover.
   ============================================================ */
.brand-logo {
...
.brand-logo:hover {
  transform: perspective(300px) rotateY(18deg) rotateX(-6deg) scale(1.08);
  animation-play-state: paused;
}
@keyframes logoFloat {
  0%, 100% { transform: perspective(300px) rotateY(0deg) translateY(0); }
  50% { transform: perspective(300px) rotateY(8deg) translateY(-2px); }
}

@media (prefers-reduced-motion: reduce) {
  .kpi-tilt, .card-motion { transform: none !important; }
