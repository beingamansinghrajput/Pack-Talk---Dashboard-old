...
export default async function handler(req, res) {
  ...
  if (!linkRow || !linkRow.active) {
    return res.status(404).send(invalidLinkHtml('This link is not recognized or has been disabled.'))
  }
  ...
  const { data: quotaMatch } = await supabase
    ...
    .limit(1)
    .maybeSingle()
  const clientRedirectTemplate = quotaMatch?.client_redirect_url || null
